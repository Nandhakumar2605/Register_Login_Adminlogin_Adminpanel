...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $errors = array();
    if (empty($email)) {
        $errors[] = '<script>alert("Email is required"); </script>';
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $errors[] = '<script>alert("Invalid email format");</script>';
    }
    if (empty($password)) {
        $errors[] = '<script>alert("password is required");</script>';
...
<head>
<meta charset="UTF-8">
<title>Own Project Admin Page</title>
...
<button class="button" type="submit">admin</button><a href="login.php">You are user </a>
</form>
